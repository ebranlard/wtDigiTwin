...
def force_interp(t,vTime,vF):
    """ Interpolate force vector known at discrete values (vTime, vF) to a given time `t` """
...
class MechSystem():
    ...
    def Force(self,t,x=None):
        if hasattr(self,'_force_ts'):
            return force_interp(t,self._time_ts,self._force_ts)
