...
try:
    from . import fatigue_tools
except:
    import fatigue_tools
...
def rainflow_windap(signal, levels=255., thresshold=(255 / 50)):
    ...
    if np.nanmax(signal) > 0:
        ...
        sig_ext = fatigue_tools.peak_trough(signal, thresshold)


        #rainflow count
        ampl_mean = fatigue_tools.pair_range_amplitude_mean(sig_ext)
...
def rainflow_astm(signal):
    ...
    sig_ext = fatigue_tools.find_extremes(signal)

    # rainflow count
    ampl_mean = np.array(fatigue_tools.rainflowcount(sig_ext))
